...
1. Asympt variance is smaller than exact variance but the boot variance is a little bit higher than exact variance.  
...
3. the 95%-tile value is larger for the wald test than for the asymptotical distribution and thus renders a higher rejection rate, which is compatible with the lower asymp variance estimte.     
...
4. The Modified Wald test gives the accurate size since its modified variance is close to the exact one and its critical value is also close to the asymptotical one.  
5. The Wald test size under the bootstrapped distribution using true betas is lower than 5% while the Wald test size under the bootstrapped distribution using estimated betas is higher than 5%. Here are two factors affecting the rejection rates: heavy tail and upperward bias. The lower one is mainly due to upperward bias while the higher one is mainly due to heavy tail. Here are some supporting evidence as follows.
...
1. Out-of-sample MSEs are larger than in-sample MSEs for both regressions, as expected.
2. In-sample MSE smaller for reg2 as expected since MSE always decreases as more variables are added.
3. Out-of-sample MSE slightly smaller for reg2 than for reg1, indicating that adding X2 does not increase the predictive power very much
...
Using the formula to compute MSE gives similar (exact) results as from the simulation. In the case of n-fold (Leave-one-out) CV, there is no randomness from the CV selection so they should match exactly.
