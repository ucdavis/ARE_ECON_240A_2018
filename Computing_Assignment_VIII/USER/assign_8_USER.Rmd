...
However, there are still two factors affecting our test size.  
(1) The randomness of our variance-covariance estimator which will lead to heavy tail of our Wald test statistics given finite sample.    
(2) The negative correlation between $\hat{\beta}_1$ and $\hat{\beta}_2$ will also lead to heavy tail of our Wald test statistics since it tends to give us a higher $W$. However, we can set the correlation between $X_1$ and $X_2$ (around $-0.255$ so that the correlation between $\hat{\beta}_1$ and $\hat{\beta}_2$ is equal to $0.3/1.1$) to remove this effect.  

In sum, our Wald test will still be overjected even though our aymptotical variance is more accurate now.
